...
from misc.misc import interval_sep, get_date_parts, print_exception_msg, get_time
...
def generate_routes(data_path):
    """
    Generates routes form input file.
    File example:
    ...
    for i in range(0, len(data) - 1):
        ...
        if 'NEW_ROUTE' in data[i] or i == len(data) - 2:
            ...
            if 'NEW_ROUTE' in next_row[0]:
                ...
                #                'time': int(row[1]),
                #                'abs_speed': int(mean(speeds)),
                #                'rel_speed': int(mean(speeds) / get_speed_limit(abs(int(row[0]))) * 100),
